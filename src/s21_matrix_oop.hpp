...
class S21Matrix : public MatrixService {
 ...
 public:
  ...
  S21Matrix() noexcept
      : rows_(0),
        cols_(0),
        matrix_(nullptr) {}  // A default constructor that initialises a matrix
                             // of some predefined dimension.
  S21Matrix(const int rows, const int cols);  // Parametrized constructor with
  ...
  }  // Adds the second matrix to the current one             //different matrix
     // dimensions.
  void SubMatrix(const S21Matrix& other) {
    S21Matrix res(*this - other);
    this->replaceMatrix(res);
  }  // Subtracts another matrix from the current one         //different matrix
     // dimensions.
  ...
  void MulMatrix(const S21Matrix& other) {
    S21Matrix res((*this) * other);
    this->replaceMatrix(res);
  }  // Multiplies the current matrix by the second matrix.   //The number of
     // columns of the first matrix is not equal to the number of rows of the
     // second matrix.
  S21Matrix Transpose() const;  // Creates a new transposed matrix from the
                                // current one and returns it.
  S21Matrix CalcComplements()
      const;  // Calculates the algebraic addition matrix of the current one and
              // returns it.  //The matrix is not square.
  double Determinant()
      const;  // Calculates and returns the determinant of the current matrix.
              // //The matrix is not square.
  S21Matrix InverseMatrix() const;  // Calculates and returns the inverse
                                    // matrix. //Matrix determinant is 0.

  // operators overload
  MatrixElement operator()(const int row, const int col)
      const;  // Indexation by matrix elements (row, column).   // Index is
              // outside the matrix.
  bool operator==(const S21Matrix& other) const {
    return this->EqMatrix(other);
  }  // Checks for matrices equality (`EqMatrix`).
  S21Matrix& operator=(
      const S21Matrix& other) noexcept;  // Assignment of values from one matrix
                                         // to another one.
  ...
  }  // Subtraction of one matrix from another.     //Different matrix
     // dimensions.
  ...
  S21Matrix operator*(const S21Matrix& other)
      const;  // Matrix multiplication        //The number of columns of the
              // first matrix does not equal the number of rows of the second
              // matrix.
  S21Matrix operator*(
      const double num) const;  // Multiplication of a matrix by a number.
  S21Matrix& operator*=(const S21Matrix& other) {
    this->MulMatrix(other);
    return *this;
  }  // Multiplication assignment (`MulMatrix`/`MulNumber`).  //-  The number of
     // columns of the first matrix does not equal the number of rows of the
     // second matrix.
