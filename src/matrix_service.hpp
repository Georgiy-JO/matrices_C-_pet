...
class MatrixService{
    public:
    inline static bool doubleEq(const double& a, const double& b)noexcept{return fabs(a-b)<EPSILON;}
    inline static bool doubleEqComplex(const double& a, const double& b){
        if(isnan(a)||isnan(b)||(isinf(a)&&isinf(b)&&((a < DOUBLE_ZERO && b < DOUBLE_ZERO) || (a > DOUBLE_ZERO && b > DOUBLE_ZERO))))
            throw DataError();
        return doubleEq(a,b);
    }
