...
class DimentionError : public MatrixError {
 public:
  DimentionError(
      const char* error =
          "Matrix sizes error: matrix dimentions should be positive.")
      : MatrixError(error) {}
};
class DataError : public MatrixError {
 public:
  DataError(const char* error =
                "Incorrect data: imposible to input / calculate due to data "
                "error (inf, nan, etc).")
      : MatrixError(error) {}
};
class DimentionEqualityError : public MatrixError {
 public:
  DimentionEqualityError(
      const char* error =
          "Calculation impossible: matrices' dimentions are not equal.")
      : MatrixError(error) {}
};
...
class MemoryAllocationError : public MatrixError {
 public:
  MemoryAllocationError(
      const char* error =
          "Memory alocation problem: congratulations, you got it somehow!")
      : MatrixError(error) {}
};
class OutOfRangeError : public MatrixError {
 public:
  OutOfRangeError(
      const char* error =
          "Out of range error: you are trying to reach element out of matrix.")
      : MatrixError(error) {}
};
...
class NonInvertibleError : public MatrixError {
 public:
  NonInvertibleError(
      const char* error =
          "The matrix is not invertable:the determinant is zero.")
      : MatrixError(error) {}
};
